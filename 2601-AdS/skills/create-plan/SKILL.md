...
Write to `plans/<issue>-<slug>/PLAN.md` following the Work Organization convention:

1. **Look for existing folder**: Check if `plans/<issue>-<slug>/` already exists with a SPEC.md
2. **Ask if missing**: If no folder matches the current work, ask for issue number and slug
3. **No issue number**: Use `0000` as placeholder (e.g., `plans/0000-feature-name/`)
4. **User override**: If user specifies a different path, use that

The spec should generally exist before the plan. If no SPEC.md exists in the target folder, suggest creating one first (or proceed if the user confirms).
...
- `CLAUDE.md` / `claude.local.md` for project-specific instructions (plans directory, conventions)
...
- Current git branch name (for plan filename)
...
**Determine output location** per the Work Organization convention:
1. Write to `plans/<issue>-<slug>/PLAN.md`
2. The folder should already exist (created during spec phase)
3. If not, ask for issue number and slug, create the folder
4. If target file exists, ask before overwriting
