...
Write to `plans/<issue>-<slug>/SPEC.md` following the Work Organization convention:

1. Ask for issue number and slug if not provided
2. If no issue number, use `0000` as placeholder (e.g., `plans/0000-user-auth/`)
3. Create the folder and an empty `LESSONS.md` alongside the spec
4. If user specifies a different path, use that
5. If file exists, ask before overwriting (unless updating)

```
plans/
  42-user-auth/
    SPEC.md       ← you write this
    LESSONS.md    ← create empty alongside
```
