...
Locate the active PLAN.md following the Work Organization convention:

1. Look in `plans/*/PLAN.md` for a plan with unchecked `[ ]` items
2. If multiple plans exist, ask the user which one to work on
3. If user specified a plan path, use that

Read the PLAN.md and locate the first unchecked `[ ]` item.
...
1. If the user specified a plan, use it.
2. Otherwise, check if a `plans/*/SPEC.md` exists and offer to generate a plan from it (suggest `create-plan` agent).
3. If nothing exists, suggest starting with `create-spec`.
...
**Delegate implementation to specialized agents.** Do not write code directly. Based on what the task requires:

| Task type | Delegate to |
|-----------|-------------|
| Backend code (services, APIs, models, jobs) | `@backend-agent` |
| Frontend code (UI, templates, styles) | `@frontend-agent` |
| SQL queries, schema changes | `@db-agent` |
...
- **Reusable learnings** (gotchas, decisions, patterns, things that didn't work): Update `LESSONS.md` in the same `plans/<issue>-<slug>/` folder
...
3. Write a final post-mortem in `LESSONS.md` (same folder as the plan):
   - What went well
   - What would change next time
   - Key takeaways
4. Move the folder to `plans/.done/`:
   ```bash
   mkdir -p plans/.done
   mv plans/<issue>-<slug> plans/.done/
   ```
