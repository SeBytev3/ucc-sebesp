...
import { LogOut, User, Menu, LayoutDashboard, Search, ClipboardList, Briefcase } from 'lucide-react';
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link to="/" className="flex-shrink-0 flex items-center">
              <span className="text-2xl font-bold text-indigo-600 tracking-tight italic">ServiMarket</span>
            </Link>
          ...
          <div className="hidden sm:ml-6 sm:flex sm:items-center space-x-6">
            <div className="flex items-center space-x-3 bg-gray-50 px-4 py-1.5 rounded-2xl border border-gray-100">
              ...
              <div className="flex flex-col">
                <span className="text-xs font-black text-gray-900 leading-tight">{user?.firstName}</span>
                <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-tighter">
                  {user?.role === 'PROVIDER' ? 'Profesional' : user?.role === 'ADMIN' ? 'Admin' : 'Cliente'}
                </span>
              </div>
            </div>
