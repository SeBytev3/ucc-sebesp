...
const statusConfig = {
  PENDING: { color: 'bg-yellow-100 text-yellow-800', icon: <Clock className="h-4 w-4" />, label: 'Pendiente' },
  ACCEPTED: { color: 'bg-blue-100 text-blue-800', icon: <CheckCircle className="h-4 w-4" />, label: 'Aceptado' },
  DECLINED: { color: 'bg-red-100 text-red-800', icon: <XCircle className="h-4 w-4" />, label: 'Rechazado' },
  COMPLETED: { color: 'bg-green-100 text-green-800', icon: <CheckCircle className="h-4 w-4" />, label: 'Completado' },
  CANCELLED: { color: 'bg-gray-100 text-gray-800', icon: <XCircle className="h-4 w-4" />, label: 'Cancelado' },
};
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 max-w-xs mx-auto">Aún no has {user?.role === 'PROVIDER' ? 'recibido' : 'realizado'} ninguna solicitud de servicio.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {requests.map((request) => {
              ...
              return (
                <div key={request.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:border-indigo-100 transition-all">
                  <div className="p-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex items-start space-x-4">
                        <div className="h-12 w-12 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600 flex-shrink-0">
                          {user?.role === 'PROVIDER' ? <User className="h-6 w-6" /> : <Briefcase className="h-6 w-6" />}
                        </div>
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold flex items-center space-x-1 ${config.color}`}>
                              {config.icon}
                              <span>{config.label}</span>
                            </span>
                          </div>
                          <div className="flex items-center text-sm text-gray-500 mt-1 space-x-4">
                            <span className="flex items-center"><Calendar className="h-3 w-3 mr-1" /> {new Date(request.requestedAt).toLocaleDateString()}</span>
                    ...
                    <div className="mt-6 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                      ...
                      {request.providerResponseNotes && (
                        <div className="mt-3 pt-3 border-t border-gray-200 flex items-start space-x-2">
                          ...
                          <div>
                            <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-wider">Respuesta del profesional:</p>
                            <p className="text-sm text-gray-600 italic">"{request.providerResponseNotes}"</p>
                          </div>
                        </div>
                      )}
                    </div>
...
const ClipboardList = ({ className }: { className: string }) => <div className={className}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="8" height="4" x="8" y="2" rx="1" ry="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/></svg></div>;
